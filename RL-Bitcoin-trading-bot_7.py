...
class CustomAgent:
    ...
    def __init__(self, lr=0.00005, epochs=1, optimizer=Adam, batch_size=32, model="", depth=0, comment=""):
        self.model = model
        ...
        self.state_size = (50, 50, 4) # 5 standard OHCL information + market and indicators
    ...
    def replay(self, states, actions, rewards, predictions, dones, next_states):
        ...
        #action, prediction = self.Actor.actor_predict(state1)
        images_states_vol = []
        images_states_price = []
        position_states = []
        for i in range(500):
            images_states_vol.append(states[i][0])
            position_states.append(states[i][1])
        images_next_states_vol = []
        images_next_states_price = []
        position_next_states = []
        for i in range(500):
            images_next_states_vol.append(next_states[i][0])
            position_next_states.append(next_states[i][1])
        images_states_vol = np.vstack(images_states_vol)
        position_states = np.vstack(position_states)
        images_next_states_vol = np.vstack(images_next_states_vol)
        position_next_states = np.vstack(position_next_states)
        actions = np.vstack(actions)
        predictions = np.vstack(predictions)

        # Get Critic network predictions
        values = self.Critic.critic_predict(images_states_vol,position_states)
        next_values = self.Critic.critic_predict(images_next_states_vol,position_next_states)
        ...
        a_loss = self.Actor.Actor.fit(x =  [images_states_vol,position_states], y = y_true, epochs=self.epochs, verbose=0, shuffle=True, batch_size=self.batch_size)
        c_loss = self.Critic.Critic.fit(x =  [images_states_vol,position_states], y = target, epochs=self.epochs, verbose=0, shuffle=True, batch_size=self.batch_size)
    ...
    def act(self, state):
        ...
        prediction = self.Actor.actor_predict(state)[0]
...
class CustomEnv:
    ...
    def reset(self, env_steps_size = 0):
        ...
        if env_steps_size > 0: # used for training dataset
            self.start_step = random.randint(50, self.df_total_steps - env_steps_size)
            self.end_step = self.start_step + env_steps_size
        else: # used for testing dataset
            self.start_step = self.lookback_window_size
            self.end_step = self.df_total_steps
        self.episode_length_passed = 0
        ...
        state = [(df_nomalized[self.start_step])[0].reshape(1, 50, 50, 4),np.expand_dims(np.array(self.orders_history),axis=0)]
        return state
    ...
    def next_observation(self):
        ...
        obs = [(self.df_normalized[self.current_step])[0].reshape(1, 50, 50, 4),np.expand_dims(np.array(self.orders_history),axis=0)]
        return obs
    ...
    def step(self, action):
        ...
        if action == 0: # Hold
            ...
            if self.position == 0:
                ...
                #print("1: "+str(self.crypto_bought))
                #print("Buy"+str(self.crypto_bought))
                #print("Buy")
                self.short_start_balance = self.balance
                ...
                #print("trade")
                self.balance = self.short_start_balance*(1-(2*self.fees)) + self.short_start_balance*((self.initial_price - current_price)/self.initial_price)
                #self.balance += (self.dollars_held - self.crypto_held * current_price) + self.dollars_held
                # print("SOLD:"+str(self.crypto_sold * current_price))
            ...
            if self.position == 1:
                ...
                #print("trade")
                # Buy with 100% of current balance
                #self.crypto_sold = self.balance / current_price
                # print("1: "+str(self.crypto_bought))
                # print("Buy"+str(self.crypto_bought))
                #print("Short")
                self.initial_price = current_price
                self.short_start_balance = self.balance
                self.balance -= self.short_start_balance
                # self.dollars_held += self.crypto_sold * current_price
                # self.dollars_held *= (1 - self.fees)  # substract fees
        ...
        if((self.position == 0) or (self.position == 1)):
            ...
            self.net_worth = self.short_start_balance*(1-(2*self.fees)) + self.short_start_balance*((self.initial_price - current_price)/self.initial_price)
        ...
        if self.net_worth <= self.initial_balance/2:
            ...
            # print("Sell trades" + str(self.sell_trades))
            # print("Buy trades" + str(self.buy_trades))
        else:
            done = False
    ...
    def get_reward(self):
        if self.episode_orders > 1 and self.episode_orders > self.prev_episode_orders:
            ...
            if self.trades[-1]['type'] == "buy" and self.trades[-2]['type'] == "sell":
                reward= self.short_start_balance*((self.initial_price - self.final_price)/self.initial_price)
                self.trades[-1]["Reward"] = reward
                return reward
            elif self.trades[-1]['type'] == "sell" and self.trades[-2]['type'] == "buy":
                #reward = self.trades[-1]['total']*self.trades[-1]['current_price'] - self.trades[-2]['total']*self.trades[-2]['current_price']
                reward = self.short_start_balance * ((self.final_price-self.initial_price) / self.initial_price)
                self.trades[-1]["Reward"] = reward
                return reward
        if self.episode_orders == 0 and self.episode_length_passed==498:
            print("0 orders")
            reward = -500
...
def train_agent(env, agent, visualize=False, train_episodes = 100, training_batch_size=500):
    agent.create_writer(env.initial_balance, env.normalize_value, train_episodes) # create TensorBoard writer
    total_average = deque(maxlen=50) # save recent 100 episodes net worth
...
if __name__ == "__main__":            
    df = pd.read_csv('TFUEL15m_test1.csv').head(7000)
    df2 = pd.read_csv('BTC15m_test1.csv').head(7000)
    df = df.drop(df.columns[[0]], axis=1)
    df = df.dropna()
    df = df.sort_values('Date')
    df_nomalized = AddIndicators(df,df2) # insert indicators to df 2021_02_21_17_54_Crypto_trader
    ...
    train_df = df[50:-test_window] # we leave 100 to have properly calculated indicators
    ...
    agent = CustomAgent(lr=0.00001, epochs=5, optimizer=Adam, batch_size=32, model="CNN", comment="Normalized")
    train_env = CustomEnv(df=train_df, df_normalized=train_df_nomalized, lookback_window_size=50)
    train_agent(train_env, agent, visualize=False, train_episodes=500000, training_batch_size=500)
    #train_multiprocessing(CustomEnv, agent, train_df, train_df_nomalized, num_worker = 30, training_batch_size=500, visualize=False, EPISODES=400000)

    #test_multiprocessing(CustomEnv, CustomAgent, test_df, test_df_nomalized, num_worker = 1, visualize=False, test_episodes=1000, folder="2021_07_24_17_08_Crypto_trader", name="1420.32_Crypto_trader", comment="3 months")
